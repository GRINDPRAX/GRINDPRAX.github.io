...
export class TelegramBotService {
  ...
  private authSessions: Map<number, { code: string; expires: number }> =
    new Map();
  // Map для временных токенов входа: token -> { telegramId, expires }
  private tempLoginTokens: Map<string, { telegramId: string; expires: number }> = new Map();

  // Генерация временного токена и сохранение
  public generateLoginToken(telegramId: string): string {
    const token = this.generateUUID();
    const expires = Date.now() + 5 * 60 * 1000; // 5 минут
    this.tempLoginTokens.set(token, { telegramId, expires });
    return token;
  }

  // Проверка и удаление токена
  public consumeLoginToken(token: string): string | null {
    const entry = this.tempLoginTokens.get(token);
    if (!entry) return null;
    if (Date.now() > entry.expires) {
      this.tempLoginTokens.delete(token);
      return null;
    }
    this.tempLoginTokens.delete(token);
  ...
  // Простой генератор UUID v4 (можно заменить на пакет uuid)
...
Для начала используйте /login для входа в систему!`;
    ...
    this.bot.onText(/\/login/, async (msg) => {
      ...
      try {
        ...
        const token = this.generateLoginToken(userId.toString());
        // Формируем ссылку для входа (замените на нужный домен, если требуется)
        const baseUrl = process.env.TELEGRAM_PUBLIC_BASE_URL || "http://evo-faceit.ru:8080";
        const loginUrl = `${baseUrl}/telegram-login/${token}`;
        await this.sendMessage(chatId, `🔐 Для входа в аккаунт перейдите по ссылке (действует 5 минут):\n<a href=\"${loginUrl}\">${loginUrl}</a>`, { parse_mode: "HTML" });
      } catch (e) {
        await this.sendMessage(chatId, "Произошла ошибка при создании ссылки для входа. Попробуйте ещё раз.");
      }
    });

    // Логика входа/создания пользователя через Telegram (примерная структура, адаптируйте под вашу базу)
      ...
      try {
        ...
        if (!user && this.config.autoCreateUsers) {
          user = createUser({
            ...
            nickname: msg.from?.username || `tg_${userId}`,
            rating: 1000,
            kills: 0,
            deaths: 0,
            kd: 0,
